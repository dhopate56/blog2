...
<div class="modal fade" id="ajax-task-modal2" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="taskCrudModal2"></h4>
      </div>
      <div class="modal-body">
        <form id="taskForm2" name="taskForm2" class="form-horizontal" enctype="multipart/form-data">
          <input type="hidden" name="task_id" id="task_id">
          <div class="form-group">
            <label for="name" class="col-sm-2 control-label">Title</label>
            <div class="col-sm-12">
...
<script>
 ...
 $(document).ready( function () {
   ...
   /* When click edit user */
    $('body').on('click', '.edit-task', function () {
      ...
      $.get('taskEdit/' + task_id, function (data) {
         ...
         $('#taskCrudModal2').html("Edit Task");
          $('#btn-save2').val("edit-task");
          $('#ajax-task-modal2').modal('show');
          $('#task_id').val(data.id);
  ...
  $('body').on('submit', '#taskForm2', function (e) {
      e.preventDefault();
